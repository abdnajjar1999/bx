...
class _CustomersScreenState extends State<CustomersScreen> {
  ...
  CustomerType _selectedType = CustomerType.shop;
  ...
  Widget build(BuildContext context) {
    return Consumer<AppProvider>(builder: (context, appProvider, child) {
      final filteredCustomers = appProvider.customers.where((c) {
        bool matchType = c.customerType == _selectedType;
        // Use a safe comparison for status
                      ...
                      onPressed: () async {
                        try {
                          final updatedCustomer =
                              customer.copyWith(status: 'مقبول');
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('خطأ في قبول الزبون: $e')),
                          );
                        }
                      },
    ...
    return Wrap(
      ...
      children: customers
          .map((customer) => _buildUserCard(
                ...
                onTapAccept: customer.status == 'معلق'
                    ? () async {
                        try {
                          final updatedCustomer =
                              customer.copyWith(status: 'مقبول');
