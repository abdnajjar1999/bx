...
class DriverDeliveryData {
  ...
  static DriverDeliveryData fromShipments(
    ...
    int partiallyCollected =
        shipments.where((s) => s.status == 'تم تحصيلها بشكل جزئي').length;
    int returned = shipments.where((s) => s.status == 'تم إرجاعها').length;
    double totalCollections = shipments.fold(0, (sum, shipment) {
      if (shipment.status == 'تم إرجاعها' ||
          shipment.status == 'بانتظار التحصيل') {
        if (shipment.receivedMoneyFromCustomer == true) {
          return sum + shipment.deliveryCost;
        }
        return sum +
            (shipment.paymentMethod == 'مدفوعة مسبقا' ? shipment.codAmount : 0);
      }
      return sum + shipment.codAmount;
    });
    double totalPrice = shipments
        .where((s) =>
            !(s.returnedAfterDelivery == false && s.status == 'تم إرجاعها'))
        .fold(0, (sum, shipment) => sum + shipment.deliveryCost);
    ...
    return DriverDeliveryData(
      ...
      price: totalPrice,
      driverPrice: shipments.fold(
          0, (sum, shipment) => sum + (shipment.driverPrice ?? 0)),
      delivered: delivered,
