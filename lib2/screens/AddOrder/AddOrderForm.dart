...
//add shelevs
  ...
  Widget _buildBulkSummary(AppProvider appProvider) {
    ...
    for (var s in shipments) {
      ...
      totalDelivery += double.tryParse(s.deliveryCostController.text) ?? 0;
    }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _buildSummaryItem(
              'إجمالي التوصيل',
              '${totalDelivery.toStringAsFixed(2)} JOD',
                    ...
                    if (shipments.length > 1) ...[
                      ...
                      IconButton(
                        ...
                        icon: Icon(
                          ...
                          size: 14,
                          color:
                              isSelected ? Colors.white70 : Colors.red.shade300,
                        ),
  ...
  Widget _buildMobileShipmentFields(ShipmentData shipment, int index) {
    return Column(
      children: [
        ...
        TextFormField(
          controller: shipment.trackingNumberController,
          focusNode: shipment.trackingNumberFocus,
          decoration: InputDecoration(
            hintText: 'رقم الإرسالية التجارية',
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(4)),
            contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12),
            isDense: false,
            hintStyle: TextStyle(fontSize: 12),
          ...
          children: [
            ...
            Expanded(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('الوجهة (المدينة/المنطقة)',
                      style: TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.bold,
                          color: Colors.grey)),
                  const SizedBox(height: 8),
                  _buildCitySearchField(shipment,
                      Provider.of<AppProvider>(context, listen: false)),
                  SizedBox(height: 12),
                  _buildRegionDropdown(shipment,
                      Provider.of<AppProvider>(context, listen: false)),
                ],
              ),
            ),
          ...
          children: [
            Expanded(
              ...
              child: _buildModernTextField(
                controller: shipment.addressDescController,
                focusNode: shipment.addressDescFocus,
                hint: 'وصف دقيق للعنوان (شارع، قريب من...)',
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              _buildModernRadio('حفظ وطباعة', 'save_and_print'),
              const SizedBox(width: 12),
              _buildModernRadio('حفظ وإغلاق', 'save_and_close'),
              const SizedBox(width: 12),
              _buildModernRadio('حفظ ومتابعة', 'save_and_continue'),
            ],
          ),
  ...
  Widget _buildModernRadio(String label, String value) {
    final isSelected = _submitAction == value;
    return Expanded(
      child: InkWell(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child:
                    const Icon(Icons.store_rounded, color: primary, size: 20),
              ),
              const SizedBox(width: 12),
              const Text(
                'بيانات المتجر المرسل',
                style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: SadradColors.textPrimary),
          ...
          SearchableDropdown<Customer>(
            label: 'اختر المتجر أو الزبون صاحب الطلبات',
            ...
            hint: 'ابحث عن اسم المتجر أو الزبون...',
          ),
        ],
      ),
    );
  ...
  void calculateDeliveryCost(AppProvider appProvider,
      {ShipmentData? specificShipment}) {
    if (appProvider.selectedCustomer == null) return;

    List<ShipmentData> targets =
        specificShipment != null ? [specificShipment] : shipments;

    for (var shipment in targets) {
      String? cityName = shipment.selectedCityPlace ?? shipment.selectedCity;
      if (cityName == null || cityName.isEmpty) continue;

      try {
        double price = appProvider.calculateDeliveryCostForCity(
            cityName, appProvider.selectedCustomer!.userid);

        setState(() {
          shipment.deliveryCostController.text =
