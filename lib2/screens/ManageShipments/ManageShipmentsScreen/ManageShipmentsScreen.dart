...
// Add this method to create the overlay
  ...
  void showFutureDetailsBottomSheet(
    ...
    bool receivedMoneyFromCustomer =
        shipment?.receivedMoneyFromCustomer ?? false;
    bool getMoneyFromUserPalance = shipment?.getMoneyFromUserPalance ??
        shipment?.returnedAfterDelivery ??
        false;
...
Widget getSubText(String status, Shipment shipment) {
  switch (status) {
    ...
    case 'على الرفوف':
      return Text('الطرد على الرف: ' + (shipment.shelfName ?? 'غير محدد'),
          style: const TextStyle(fontSize: 11));
    case 'تم إرجاعها':
      return Column(
        mainAxisSize: MainAxisSize.min,
        ...
        children: [
          Text('مع: ${shipment.orderPossession.nameAr}',
              style: const TextStyle(fontSize: 11)),
          if (shipment.returnedAfterDelivery != null)
            Text(
              shipment.returnedAfterDelivery == true
                  ? 'بعد الوصول'
                  : 'قبل الوصول',
              style: const TextStyle(
                fontSize: 11,
